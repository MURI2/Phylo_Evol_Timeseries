...
sdata.all <- c("data/gene_by_sample/", strain, "/sample_by_gene_Gscore.txt")
...
png(filename = paste(c("figs/pcoa/D100_G", strain, ".png"), collapse = ''),
...
df.all.no0.D00 <- df.all.no0[grep("D100", rownames(df.all.no0)), ]
treats.D300 <- c()
...
# Indicator Value
indval <- multipatt(df.all.no0.D00, cluster = treats.D300, func = "IndVal.g", control = how(nperm=999)) 
summary(indval)
